...
⁠   speed_val ⁠ and ⁠ direction_val ⁠ are 10-bit (0-1023) already mapped.
    """
    data = bytearray(10)
    data[0] = 0x60 # General information packet type: 1th bit CLEAR, then length 6 bytes (0b110), then packet type 0 (0b000) -> 0x60

    # MSB 7 bits of 10-bit speed_val, direction_val, and max_speed_const
    data[1] = 0x80 | ((speed_val     >> 3) & 0x7F) # Get the 7 MSB of speed_val (out of 10), and SET the 8th bit to 1
    data[2] = 0x80 | ((direction_val >> 3) & 0x7F) # Get the 7 MSB of direction_val (out of 10), and SET the 8th bit to 1
    data[3] = 0x80 | ((max_speed_const >> 1) & 0x7F) # Get the 7 MSB of max_speed_const (out of 8), and SET the 8th bit to 1

    # LSB packing (see original comment)
    data[4] = (
...
def map(low_in, high_in, low_out, high_out, value):
    """
...
if __name__ == "__main__":
    ...
    while True:
        speed_byte = map(-1.0, 1.0, 0, 1023, speed)
        direction_byte = map(-1.0, 1.0, 0, 1023, direction)
